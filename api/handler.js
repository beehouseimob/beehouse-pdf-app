...
function drawHeader(doc) {
    ...
    doc.text('www.beehouse.imb.br | Fone: (47) 99287-9066', rightAlignX, initialY + 36, { width: blockWidth, align: 'left' });
...
async function generatePdfPromise(data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const xMarkOffset = checkSize / 2; // Metade do tamanho da caixa
            if (temExclusividade) {
                // Desenha X no SIM
                doc.text('X', xSim, yCheck - 1 , { // -1 ajuste vertical
                     width: checkSize, 
                     height: checkSize, 
                     align: 'center', 
                     lineGap: -3 // Ajuste fino vertical
                    });
            } else {
                 // Desenha X no NÃO
                doc.text('X', xNao, yCheck - 1, { 
                    width: checkSize, 
                    height: checkSize, 
                    align: 'center',
                    lineGap: -3 
